...
* Sub Research Question: Which of the continent has less change in change of climate?
...
```{r}
## line graph/ continent 별로 나눠서 다 따로 그려야함
...
# data 를 여기서 왜 data_with_continent 를 썼는지...?  + 다 continent 로 나눠서 그렸으면 좋겠음
ggplot(data.frame(data_with_continent), aes(x = Extreme_Weather_Events)) +  
  # bar graph 가 더 낳은 것 같음
   geom_bar(binwidth = 0.5, fill = "skyblue", color = "black") + 
    labs(
...
```{r}
# 이거 너무 지저분함 그래프를 대륙별로 다 따로 그려야함 
...
ggplot(data, aes(x = Year, y = Total_Precipitation_mm, fill = Continent)) + 
  #대륙별로 다시 그리고 line 으로도 그려보고 지저분하면 bar 로 그리길 바람.
