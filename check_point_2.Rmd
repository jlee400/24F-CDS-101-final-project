...
```{r, fig.size = 40}
ggplot(aggregated_data, aes(x = Year, y =  avg_co2_emissions_mt, 
                            fill = Continent)) +
...
```{r}
ggplot(aggregated_data, aes(x = Year, y = avg_total_precipitation_mm, fill = Continent)) +
...
```
```{r}
ggplot(slopes_resilience, aes(x = reorder(Crop_Type, abs(slope)), y = slope, fill = slope > 0)) +
...
slopes_resilience1 <- aggregated_data %>%
  group_by(Crop_Type) %>%
  summarize(
    slope = abs(coef(lm(avg_crop_yield ~ avg_co2_emissions_mt, data = cur_data()))[2]), 
    intercept = coef(lm(avg_crop_yield ~ avg_co2_emissions_mt, data = cur_data()))[1],
  ) %>%
  arrange(desc(slope)) 
...
```
```{r}
ggplot(slopes_resilience1, aes(x = reorder(Crop_Type, abs(slope)), y = slope, fill = slope > 0)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  coord_flip() +
...
slopes_resilience2 <- aggregated_data %>%
  group_by(Crop_Type) %>%
  summarize(
    slope = coef(lm(avg_crop_yield ~ avg_extreme_weather_events, data = cur_data()))[2], 
...
ggplot(correlation_matrix, aes(x = Crop_Type, y = "Correlation", fill = correlation)) +
  geom_tile() +
  labs(
...
slopes <- aggregated_data %>%
  group_by(Continent) %>%
  summarise(
    slope = abs(coef(lm(avg_soil_health_index ~ avg_fertilizer_use_kg_per_ha))[2]), 
...
slopes_pesticide <- aggregated_data %>%
  group_by(Continent) %>%
  summarise(
    slope = abs(coef(lm(avg_soil_health_index ~ avg_pesticide_use_kg_per_ha))[2]), 
    p_value = summary(lm(avg_soil_health_index ~ 
...
ggplot(sampled_data_by_continent, aes(x = Soil_Health, y = avg_crop_yield)) +
  ...
  scale_color_brewer(palette = "Set2") +  
  theme_minimal() +
  facet_wrap(~ Crop_Type, scales = "free") # Facet by Country
...
```{r}
ggplot(test_df, aes(x = predicted_yield, y = predicted_economic_impact, color = Continent)) +
...
slope_intercept <- test_df %>%
  group_by(Continent) %>%
  summarize(
    slope = coef(lm(predicted_economic_impact ~ predicted_yield, data = cur_data()))[2],
    intercept = coef(lm(predicted_economic_impact ~ predicted_yield, data = cur_data()))[1]
  )
...
```{r}
ggplot(test_df, aes(x = predicted_yield, y = predicted_economic_impact, color = Crop_Type)) +
  geom_point(size = 3, alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed") +
  labs(
    title = "Predicted Economic Impact vs Predicted Crop Yield",
